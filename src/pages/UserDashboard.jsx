...
// Updated RightDescriptionPanel with submodels dropdown
const RightDescriptionPanel = React.memo(({ selectedProject, selectedModel, onModelChange, darkMode }) => {
  const [collapsed, setCollapsed] = useState(false);
  const [expanded, setExpanded] = useState(false);
  const [selectedSubModel, setSelectedSubModel] = useState("main");

  if (!selectedProject) return null;

  const hasMainModel = !!selectedProject.modelFileId;
  const hasSubModels = selectedProject.subModels?.length > 0;

  const getCurrentModelDescription = () => {
    if (selectedSubModel === "main") {
      return selectedProject.modelDescription || selectedProject.description || 'No description available.';
    } else {
      const subModel = selectedProject.subModels?.find(sm => sm.fileId === selectedSubModel);
      return subModel?.description || 'No description available.';
    }
  };

  const desc = getCurrentModelDescription();
  const shortDesc = useMemo(() =>
    desc.length > 300 ? desc.slice(0, 300) + '...' : desc,
    [desc]
  );

  const handleSubModelChange = (modelId) => {
    setSelectedSubModel(modelId);
    if (modelId === "main") {
      onModelChange({
        url: `http://localhost:5000/api/projects/file/${selectedProject.modelFileId}`,
        name: selectedProject.modelFileName || "Main Model"
      });
    } else {
      const subModel = selectedProject.subModels?.find(sm => sm.fileId === modelId);
      if (subModel) {
        onModelChange({
          url: `http://localhost:5000/api/projects/file/${subModel.fileId}`,
          name: subModel.fileName || "Sub Model"
        });
      }
    }
  };
      ...
      {!collapsed && (
        <div className="p-6 overflow-y-auto">
          <h3 className={`text-xl font-bold mb-4 ${darkMode ? 'text-purple-400' : 'text-purple-700'}`}>
            {selectedModel?.name || "3D Model"}
          </h3>

          {(hasMainModel && hasSubModels) && (
            <div className="mb-4">
              ...
              <select
                ...
                className={`w-full px-3 py-2 rounded-lg border backdrop-blur-md ${darkMode
                  ? 'bg-gray-800/50 border-gray-600 text-white'
                  : 'bg-white/50 border-gray-300 text-gray-900'
                  } focus:outline-none focus:ring-2 focus:ring-purple-500`}
              >
                <option value="main">Main Model</option>
                {selectedProject.subModels?.map((subModel) => (
                  <option key={subModel.fileId} value={subModel.fileId}>
                    {subModel.fileName || `Model ${subModel.fileId}`}
                  </option>
                ))}
              </select>
            </div>
          ...
          <p className="text-sm leading-relaxed mb-4">{expanded ? desc : shortDesc}</p>

          {desc.length > 300 && (
            <button
              onClick={() => setExpanded(!expanded)}
              className={`text-sm font-medium px-4 py-2 rounded-lg transition-colors backdrop-blur-md ${darkMode ? 'bg-purple-800/50 hover:bg-purple-700/50 text-purple-300' : 'bg-purple-100/50 hover:bg-purple-200/50 text-purple-700'
                }`}
            >
...
// Simple Build Card (shows only name and info icon)
// Simple Build Card with description
const SimpleBuildCard = React.memo(({ build, onClick, darkMode }) => {
  return (
    <div
      className={`group p-6 rounded-3xl cursor-pointer transition-all duration-300 border-2 backdrop-blur-xl h-full min-h-[220px] flex flex-col justify-between ${darkMode
        ? 'border-purple-500/30 bg-white/5 hover:border-purple-400/60 hover:bg-white/10 hover:shadow-2xl hover:shadow-purple-500/30'
        : 'border-purple-300/60 bg-white/30 hover:border-purple-400/80 hover:bg-white/40 hover:shadow-2xl hover:shadow-purple-200'
        } hover:scale-[1.02]`}
      onClick={onClick}
    >
      <div className="flex-1 flex flex-col">
        <div className="flex items-start justify-between mb-3">
          <h4 className={`text-xl font-black flex-1 break-words bg-gradient-to-r from-purple-400 to-blue-400 bg-clip-text text-transparent mr-3`}>
            {build.name}
          </h4>
          <div className={`p-3 rounded-xl backdrop-blur-md ${darkMode
        ...
        <p className={`text-sm leading-relaxed line-clamp-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'} mb-4 flex-1`}>
          {build.description || "No description available for this build."}
        </p>

        {build.isMain && (
          <span className="px-3 py-1 bg-gradient-to-r from-purple-400 to-blue-400 text-white text-xs font-bold rounded-full shadow-lg inline-block self-start">
            Main Build
          </span>
        )}
      </div>
...
// Expanded Build Card (full width when expanded)
const ExpandedBuildCard = React.memo(({ build, project, darkMode, onLaunch, onVR, launchingBuilds, launchingVRBuilds, onClose }) => {
  return (
    <div className={`p-8 rounded-3xl backdrop-blur-xl border-2 shadow-2xl w-full ${darkMode
      ? 'border-purple-500/30 bg-gray-900/30'
      : 'border-purple-300/60 bg-white/30'
      }`}>
      <div className="flex items-start justify-between mb-6">
        <div className="flex-1">
          <div className="flex items-center gap-4 mb-4">
            <h4 className={`text-2xl font-black break-words bg-gradient-to-r from-purple-400 to-blue-400 bg-clip-text text-transparent`}>
              {build.name}
            </h4>
            {build.isMain && (
              <span className="px-4 py-2 bg-gradient-to-r from-purple-400 to-blue-400 text-white text-sm font-bold rounded-full shadow-lg">
                Main Build
              </span>
            )}
          </div>
          <p className={`text-lg leading-relaxed break-words ${darkMode ? 'text-gray-300' : 'text-gray-800'} mb-6`}>
            {build.description || "No description available"}
          </p>
        </div>
        <button
          onClick={onClose}
          className={`p-3 rounded-xl backdrop-blur-md ml-4 ${darkMode
      ...
      <div className="flex gap-4 max-w-md">
        <button
          onClick={(e) => { e.stopPropagation(); onLaunch(project, build); }}
          disabled={launchingBuilds[build._id]}
          className={`flex-1 py-4 rounded-2xl font-bold text-white border transition-all shadow-lg hover:shadow-xl bg-gradient-to-r from-purple-400 to-blue-400 hover:from-purple-500 hover:to-blue-500 border-purple-500/30 ${launchingBuilds[build._id] ? 'opacity-50 cursor-not-allowed' : ''
            }`}
        >
          {launchingBuilds[build._id] ? 'Launching...' : 'Launch Build'}
        </button>
        <button
          onClick={(e) => { e.stopPropagation(); onVR(project, build); }}
          disabled={launchingVRBuilds[build._id]}
          className={`flex-1 py-4 rounded-2xl font-bold text-white border transition-all shadow-lg hover:shadow-xl bg-gradient-to-r from-purple-400 to-blue-400 hover:from-purple-500 hover:to-blue-500 border-purple-500/30 ${launchingVRBuilds[build._id] ? 'opacity-50 cursor-not-allowed' : ''
            }`}
        >
          {launchingVRBuilds[build._id] ? 'Preparing...' : 'VR Mode'}
        </button>
      </div>
    </div>
  );
...
// Model Card (same size as build cards)
const ModelCard = React.memo(({ project, darkMode, onOpenModel }) => {
  return (
    <div className={`p-6 rounded-3xl backdrop-blur-xl border-2 shadow-2xl hover:shadow-3xl transition-all hover:scale-[1.02] min-h-[280px] border-purple-500/30 cursor-pointer flex flex-col justify-between h-full`}
      onClick={() => onOpenModel(project.modelFileId, project.modelFileName)}>
      <div>
        <div className="flex items-start justify-between mb-4">
          <h4 className={`text-lg font-black flex-1 break-words bg-gradient-to-r from-purple-400 to-blue-400 bg-clip-text text-transparent`}>
            {project.modelFileName || "Main Model"}
          </h4>
          <span className="px-3 py-2 bg-gradient-to-r from-purple-400 to-blue-400 text-white text-sm font-bold rounded-full shadow-lg flex-shrink-0 ml-2">
            Main
          </span>
        </div>
        <div className="mb-4">
          <p className={`text-sm leading-relaxed break-words overflow-hidden line-clamp-4 ${darkMode ? 'text-gray-300' : 'text-gray-800'}`}>
            {project.modelDescription || "High-quality 3D model ready for interactive visualization and detailed exploration."}
          </p>
        </div>
      </div>
      <div className={`w-full py-3 rounded-2xl font-bold text-white border transition-all shadow-lg hover:shadow-xl bg-gradient-to-r from-purple-400 to-blue-400 hover:from-purple-500 hover:to-blue-500 border-purple-500/30 flex items-center justify-center gap-3`}>
        <View size={20} />
        View Details
      </div>
    </div>
...
export default function UserDashboard() {
  ...
  const handleProjectSelect = (project) => {
    ...
    setExpandedBuild(null); // Close any expanded build when switching projects
    setUrlParam("project", project._id);
  };

  const handleBuildClick = (build) => {
    setExpandedBuild(build._id === expandedBuild ? null : build._id);
    ...
    <div className="flex flex-col h-screen transition-colors duration-300 relative overflow-hidden">
      <div className="relative z-10 flex flex-col h-full">
        ...
        <main className="flex flex-1 overflow-hidden">
            ...
            <div className="fixed inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm z-50">
              <div className="bg-white/90 dark:bg-gray-800/90 rounded-2xl p-8 shadow-2xl flex flex-col items-center">
                ...
                <p className="text-lg font-semibold text-gray-800 dark:text-white">
                  {loading ? "Loading projects..." : "Restoring view..."}
                </p>
              </div>
            </div>
          )}
          ...
          {!selectedProject && (
            <div className={`w-80 border-r flex flex-col ${darkMode ? 'border-gray-700 bg-gray-900/30' : 'border-gray-200 bg-white/30'} backdrop-blur-xl`}>
              <div className="flex-1 overflow-y-auto">
                {filteredProjects.length > 0 ? (
                  <div className="p-2">
                    {filteredProjects.map((project) => (
                      <div
                        ...
                        className={`p-3 mx-2 rounded-lg cursor-pointer transition-all ${darkMode
                          ? 'hover:bg-purple-600/20 hover:text-purple-300 text-gray-300'
                          : 'hover:bg-purple-100 hover:text-purple-700 text-gray-700'
                          } group`}
                      >
                        <div className="flex items-center gap-3">
                          <div className={`p-2 rounded-lg backdrop-blur-md ${darkMode ? 'bg-purple-600/30' : 'bg-purple-100'
                            }`}>
                            <FolderPlus size={16} className={darkMode ? 'text-purple-400' : 'text-purple-600'} />
                          </div>
                  ...
                  <div className="flex flex-col items-center justify-center p-8 text-center">
                    <FolderPlus size={32} className={darkMode ? 'text-gray-600' : 'text-gray-400'} />
          ...
          <div className="flex-1 overflow-y-auto">
            {selectedProject ? (
              <div className="flex flex-col min-h-full relative p-8">
                <div className="relative z-10">
                  {/* Project Header - Fixed Description Layout */}
                  <div className={`mb-8 p-8 rounded-3xl backdrop-blur-md shadow-2xl border-2 transition-all ${darkMode ? 'border-purple-600/30' : 'border-purple-300/60'}`}>
                    <div className="flex items-start justify-between gap-6">
                      <div className="flex-1 min-w-0">
                        <h2 className={`text-3xl font-black mb-4 bg-gradient-to-r from-purple-400 to-blue-400 bg-clip-text text-transparent drop-shadow-sm`}>
                          {selectedProject.name}
                        </h2>
                        <p className={`text-lg leading-relaxed ${darkMode ? 'text-gray-300' : 'text-gray-800'} break-words whitespace-normal line-clamp-4`}>
                          {selectedProject.description || "No description available"}
                        </p>
                      </div>
                      <div className={`px-6 py-3 rounded-2xl text-sm font-bold backdrop-blur-lg shadow-lg flex-shrink-0 bg-gradient-to-r from-purple-400 to-blue-400 text-white border border-purple-500/30`}>
                  ...
                  {/* === BUILDS SECTION === */}
                  {selectedProject.builds?.length > 0 && (
                    <div className="mb-12">
                      <h3 className={`text-2xl font-black mb-6 bg-gradient-to-r from-purple-400 to-blue-400 bg-clip-text text-transparent`}>
                        Builds ({selectedProject.builds.length})
                      </h3>

                      {/* If a build is expanded, show only that build in full width */}
                      {expandedBuild ? (
                        <div className="w-full">
                          {selectedProject.builds.map((build) => (
                            build._id === expandedBuild && (
                              <ExpandedBuildCard
                                key={build._id}
                                build={build}
                                project={selectedProject}
                                darkMode={darkMode}
                                onLaunch={launchBuildExecutable}
                                onVR={handleProjectVR}
                                launchingBuilds={launchingBuilds}
                                launchingVRBuilds={launchingVRBuilds}
                                onClose={() => setExpandedBuild(null)}
                              />
                            )
                          ))}
                        </div>
                      ) : (
                        /* Show builds in grid - full width for 1, half width for 2, third for 3+ */
                        <div className={`grid gap-6 ${selectedProject.builds.length === 1 ? 'grid-cols-1' :
                          selectedProject.builds.length === 2 ? 'grid-cols-1 md:grid-cols-2' :
                  ...
                  {/* === MODELS SECTION - Only show when no build is expanded === */}
                  {!expandedBuild && (
                    <div className="mb-12">
                      <h3 className={`text-2xl font-black mb-6 bg-gradient-to-r from-purple-400 to-blue-400 bg-clip-text text-transparent`}>
                        3D Models
                      ...
                      {selectedProject.modelFileId ? (
                        <div className="w-1/3">
                          <ModelCard
                            project={selectedProject}
                            darkMode={darkMode}
                            onOpenModel={openModelPopup}
                          />
                        </div>
                      ) : (
                        <div className="flex flex-col items-center justify-center py-12 rounded-3xl backdrop-blur-xl border-2 border-purple-500/30 min-h-[200px]">
                          <View size={48} className={`mb-4 ${darkMode ? 'text-gray-600' : 'text-gray-400'}`} />
                          <h4 className={`text-lg font-bold mb-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                            No 3D Models Available
                          </h4>
                          <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'} text-center max-w-md`}>
                  ...
                  {/* Show message if no content and no build is expanded */}
                  {!expandedBuild && !selectedProject.builds?.length && !selectedProject.modelFileId && (
                    <div className="flex flex-col items-center justify-center py-16 rounded-3xl backdrop-blur-xl border-2 border-purple-500/30 min-h-[400px]">
                      <FolderPlus size={64} className={`mb-4 ${darkMode ? 'text-gray-600' : 'text-gray-400'}`} />
                      <h3 className={`text-2xl font-bold mb-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                        No Content Available
                      </h3>
                      <p className={`text-lg ${darkMode ? 'text-gray-400' : 'text-gray-600'} text-center max-w-md`}>
                        This project doesn't contain any builds or 3D models yet.
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="fixed bottom-6 right-6 z-50">
                  <button
                    ...
                    className={`px-6 py-3 rounded-2xl font-bold text-sm shadow-2xl backdrop-blur-lg flex items-center gap-2 transition-all hover:scale-105 bg-gradient-to-r from-purple-400 to-blue-400 hover:from-purple-500 hover:to-blue-500 text-white border border-purple-500/30`}
                  >
                    Back to Projects
                  </button>
                </div>
              ...
              <div className="flex-1 flex items-center justify-center p-8">
                <div className={`text-center max-w-md rounded-2xl p-8 backdrop-blur-xl ${darkMode
                  ? 'bg-gray-800/30 border border-gray-700'
                  : 'bg-white/30 border border-gray-200'
                  }`}>
                  <Grid3X3 size={64} className={`mx-auto mb-4 ${darkMode ? 'text-gray-600' : 'text-gray-400'
                    }`} />
                  ...
                  <p className={darkMode ? 'text-gray-400' : 'text-gray-600'}>
                    {filteredProjects.length > 0
                      ? "Select a project from the sidebar to view details, 3D models, and builds."
                      : "Get started by creating your first project to explore 3D models and VR experiences."
                    }
                  </p>
                </div>
              </div>
      ...
      {showRedirectOverlay && (
        <RedirectOverlay darkMode={darkMode} onClose={handleRedirectCancel} />
      )}

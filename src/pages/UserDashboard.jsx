...
export default function UserDashboard() {
  ...
  const [launchingBuilds, setLaunchingBuilds] = useState({});
  const [launchingVRBuilds, setLaunchingVRBuilds] = useState({});
  const [projectViewMode, setProjectViewMode] = useState("overview"); // "overview" | "builds" | "models"
  const handleProjectVR = async (project, build = null) => {
    // If no specific build provided, use the main build
    const targetBuild = build || project.builds?.find(b => b.isMain);
    const buildId = targetBuild?._id;
  ...
  useEffect(() => {
    if (selectedProject && projectViewMode === "overview") {
      const hasMainModel = !!selectedProject.modelFileId;
      const hasSubModels = selectedProject.subModels?.length > 0;
      const hasModels = hasMainModel || hasSubModels;
      const hasBuilds = selectedProject.builds?.length > 0;


      if (hasBuilds && !hasModels) {
        setProjectViewMode("builds");
      }
    }
  }, [selectedProject, projectViewMode]);
  const launchVrBuild = async (project) => {
    try {
      setLoading(true);
    ...
    <div className="flex flex-col h-screen transition-colors duration-300 relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        <main className="flex flex-1 overflow-hidden">
          ...
          {/* Projects Sidebar - Always visible when no project is selected */}
          {!selectedProject && (
          ...
          <div className="flex-1 overflow-y-auto">
            {selectedProject ? (
              /* Project Details View - Keep this exactly the same */
                ...
                <div className="relative z-10">
                  ...
                  {projectViewMode === "overview" && (
                    <div className={`grid gap-6 max-w-5xl mx-auto px-4 ${(() => {
                        ...
                        if (!hasModels && !hasBuilds) {
                          return 'grid-cols-1 max-w-md';
                        } else if (hasModels && hasBuilds) {
                      ...
                      {(() => {
                        ...
                        return (
                          <button
                            onClick={() => setProjectViewMode("builds")}
                            className={`group relative h-64 rounded-3xl overflow-hidden backdrop-blur-lg border-2 transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl ${darkMode
                              ? 'border-purple-500/40 hover:border-purple-400/60'
                              : 'border-purple-300/60 hover:border-purple-400/80'
                              }`}
                          >
                            <div className="absolute inset-0 bg-gradient-to-br from-purple-400/10 to-blue-400/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                            <div className="relative z-10 h-full flex flex-col items-center justify-center p-8">
                              <div className={`w-20 h-20 rounded-3xl flex items-center justify-center mb-6 shadow-2xl transition-transform duration-300 group-hover:scale-110 bg-gradient-to-r from-purple-400 to-blue-400`}>
                                <Monitor size={40} className="text-white drop-shadow-sm" />
                              </div>
                              <h3 className={`text-2xl font-black mb-3 bg-gradient-to-r from-purple-400 to-blue-400 bg-clip-text text-transparent`}>
                                Builds
                              </h3>
                              <p className={`text-lg font-semibold ${darkMode ? 'text-gray-300' : 'text-gray-800'}`}>
                                {selectedProject.builds?.length || 0} Build{(selectedProject.builds?.length || 0) !== 1 ? 's' : ''}
                              </p>
                            </div>
                          </button>
                        );
                      ...
                      {(() => {
                        ...
                        return (
                          <button
                            onClick={() => setProjectViewMode("models")}
                            className={`group relative h-64 rounded-3xl overflow-hidden backdrop-blur-lg border-2 transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl ${darkMode
                              ? 'border-purple-500/40 hover:border-purple-400/60'
                              : 'border-purple-300/60 hover:border-purple-400/80'
                              }`}
                          >
                            ...
                            <div className="relative z-10 h-full flex flex-col items-center justify-center p-8">
                              <div className={`w-20 h-20 rounded-3xl flex items-center justify-center mb-6 shadow-2xl transition-transform duration-300 group-hover:scale-110 bg-gradient-to-r from-purple-400 to-blue-400`}>
                                <View size={40} className="text-white drop-shadow-sm" />
                              </div>
                              <h3 className={`text-2xl font-black mb-3 bg-gradient-to-r from-purple-400 to-blue-400 bg-clip-text text-transparent`}>
                                3D Models
                              </h3>
                              <p className={`text-lg font-semibold ${darkMode ? 'text-gray-300' : 'text-gray-800'}`}>
                                {(() => {
                                  const totalModels = (hasMainModel ? 1 : 0) + (hasSubModels ? selectedProject.subModels.length : 0);
                  ...
                  {projectViewMode === "builds" && selectedProject.builds?.length > 0 && (
                    <div className="w-full px-4">
                      <div className={`
                grid gap-6
                ${selectedProject.builds.length === 1 ? 'grid-cols-1 max-w-2xl mx-auto' : ''}
                ${selectedProject.builds.length === 2 ? 'grid-cols-1 md:grid-cols-2' : ''}
                ${selectedProject.builds.length === 3 ? 'grid-cols-1 md:grid-cols-3' : ''}
                ${selectedProject.builds.length >= 4 ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4' : ''}
              `}>
                        {selectedProject.builds.map((build) => (
                          <div
                            key={build._id}
                            className="flex flex-col h-full p-6 rounded-3xl backdrop-blur-xl border-2 shadow-2xl hover:shadow-3xl transition-all hover:scale-[1.02] min-h-[280px] border-purple-500/30"
                          >
                            <div className="flex items-start justify-between mb-4">
                              <h4 className={`text-xl font-black flex-1 break-words bg-gradient-to-r from-purple-400 to-blue-400 bg-clip-text text-transparent`}>
                                {build.name}
                              </h4>
                              {build.isMain && (
                                <span className="px-3 py-1 bg-gradient-to-r from-purple-400 to-blue-400 text-white text-xs font-bold rounded-full shadow-lg flex-shrink-0 ml-2">
                                  Main
                                </span>
                              )}
                            </div>
                            <div className="flex-1 min-h-[48px] mb-4">
                              <p className={`text-base leading-relaxed break-words overflow-hidden line-clamp-2 ${darkMode ? 'text-gray-300' : 'text-gray-800'}`}>
                                {build.description || "No description available"}
                              </p>
                            </div>
                            <div className="flex gap-3 mt-auto pt-4 border-t border-purple-400/20">
                              <button
                                onClick={(e) => { e.stopPropagation(); launchBuildExecutable(selectedProject, build); }}
                                disabled={launchingBuilds[build._id]}
                                className={`flex-1 py-3 rounded-2xl font-bold text-white border transition-all shadow-lg hover:shadow-xl bg-gradient-to-r from-purple-400 to-blue-400 hover:from-purple-500 hover:to-blue-500 border-purple-500/30 ${launchingBuilds[build._id] ? 'opacity-50 cursor-not-allowed' : ''
                                  }`}
                              >
                                {launchingBuilds[build._id] ? 'Launching...' : 'Launch'}
                              </button>
                              <button
                                onClick={(e) => { e.stopPropagation(); handleProjectVR(selectedProject, build); }}
                                disabled={launchingVRBuilds[build._id]}
                                className={`flex-1 py-3 rounded-2xl font-bold text-white border transition-all shadow-lg hover:shadow-xl bg-gradient-to-r from-purple-400 to-blue-400 hover:from-purple-500 hover:to-blue-500 border-purple-500/30 ${launchingVRBuilds[build._id] ? 'opacity-50 cursor-not-allowed' : ''
                                  }`}
                              >
                      ...
                      <div className="flex justify-end mt-8 mb-8">
                        <button
                          ...
                          className={`px-8 py-4 rounded-2xl font-bold text-lg shadow-2xl backdrop-blur-lg flex items-center gap-2 transition-all hover:scale-105 bg-gradient-to-r from-purple-400 to-blue-400 hover:from-purple-500 hover:to-blue-500 text-white border border-purple-500/30`}
                        >
                          Back to Overview
                        </button>
                      </div>
                    ...
                    <div className="w-full px-4">
                      {(() => {
                        ...
                        if (totalModels === 0) {
                          return (
                            <div className="flex flex-col items-center justify-center py-16 rounded-3xl backdrop-blur-xl border-2 border-purple-500/30 min-h-[400px]">
                              <View size={64} className={`mb-4 ${darkMode ? 'text-gray-600' : 'text-gray-400'}`} />
                              <h3 className={`text-2xl font-bold mb-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                        ...
                        return (
                          <>
                            <div className={`
                      grid gap-6
                      ${totalModels === 1 ? 'grid-cols-1 max-w-2xl mx-auto' : ''}
                      ${totalModels === 2 ? 'grid-cols-1 md:grid-cols-2' : ''}
                      ${totalModels === 3 ? 'grid-cols-1 md:grid-cols-3' : ''}
                      ${totalModels >= 4 ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4' : ''}
                    `}>
                              {hasMainModel && (
                                <div className="flex flex-col h-full p-6 rounded-3xl backdrop-blur-xl border-2 shadow-2xl hover:shadow-3xl transition-all hover:scale-[1.02] min-h-[280px] border-purple-500/30">
                                  <div className="flex items-start justify-between mb-4">
                                    <h4 className={`text-xl font-black flex-1 break-words bg-gradient-to-r from-purple-400 to-blue-400 bg-clip-text text-transparent`}>
                                      {selectedProject.modelFileName || "Main Model"}
                                    </h4>
                                    <span className="px-3 py-1 bg-gradient-to-r from-purple-400 to-blue-400 text-white text-xs font-bold rounded-full shadow-lg flex-shrink-0 ml-2">
                                      Main
                                    </span>
                                  ...
                                  <div className="flex-1 min-h-[48px] mb-4">
                                    <p className={`text-base leading-relaxed break-words overflow-hidden line-clamp-2 ${darkMode ? 'text-gray-300' : 'text-gray-800'}`}>
                                      {selectedProject.modelDescription || "High-quality 3D model ready for interactive visualization and detailed exploration."}
                                    </p>
                                  </div>
                                  <button
                                    onClick={() => openModelPopup(selectedProject.modelFileId, selectedProject.modelFileName)}
                                    className={`w-full py-3 rounded-2xl font-bold text-white border transition-all shadow-lg hover:shadow-xl bg-gradient-to-r from-purple-400 to-blue-400 hover:from-purple-500 hover:to-blue-500 border-purple-500/30 flex items-center justify-center gap-3 mt-auto`}
                                  >
                                    <View size={22} />
                                    View Model
                                  </button>
                              ...
                              {hasSubModels && selectedProject.subModels?.map((subModel, i) => (
                                <div
                                  key={subModel._id || i}
                                  className="flex flex-col h-full p-6 rounded-3xl backdrop-blur-xl border-2 shadow-2xl hover:shadow-3xl transition-all hover:scale-[1.02] min-h-[280px] border-purple-500/30"
                                >
                                  <h4 className={`text-xl font-black mb-4 break-words bg-gradient-to-r from-purple-400 to-blue-400 bg-clip-text text-transparent`}>
                                    {subModel.fileName || `Model ${i + 1}`}
                                  </h4>
                                  <div className="flex-1 min-h-[48px] mb-4">
                                    <p className={`text-base leading-relaxed break-words overflow-hidden line-clamp-2 ${darkMode ? 'text-gray-300' : 'text-gray-800'}`}>
                                      {subModel.description || "Detailed 3D model for immersive viewing, analysis, and interactive exploration."}
                                    </p>
                                  </div>
                                  <button
                                    onClick={() => openModelPopup(subModel.fileId, subModel.fileName)}
